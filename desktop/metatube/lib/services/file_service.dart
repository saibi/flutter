...
class FileService {
  ...
  void saveContent(context) async {
    ...
    try {
      if (_selectedFile != null) {
        ...
        final filePath = '$metadataDirPath/${todayDate}_${title}_metadata.txt';
        final newFile = File(filePath);
        await newFile.writeAsString(textContent);
      }
      SnackBarUtils.showStackBar(
  ...
  void loadFile(context) async {
    try {
      ...
      if (result != null) {
        ...
        SnackBarUtils.showStackBar(context, Icons.upload_file, 'File uploaded');
      } else {
        SnackBarUtils.showStackBar(
  ...
  void newFile(context) {
    ...
    SnackBarUtils.showStackBar(context, Icons.file_upload, 'New file created');
  }
  ...
  void newDirectory(context) async {
    try {
      ...
      _selectedFile = null;
      SnackBarUtils.showStackBar(context, Icons.folder, 'New folder selected');
    } catch (_) {
      SnackBarUtils.showStackBar(
